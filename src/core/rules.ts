import { AgentCard, SecurityFinding, SeverityLevel } from '../types';
 ...
 */
export function checkSignaturesPresent(card: AgentCard): SecurityFinding[] {
 ...
 */
export function checkSignatureIntegrity(card: AgentCard): SecurityFinding[] {
  const findings: SecurityFinding[] = [];
  const signatures = card.signatures;
  ...
  let providerDomain = '';
  try {
    if (providerUrl) providerDomain = new URL(providerUrl).hostname.toLowerCase();
  } catch {
 ...
 */
export function checkTransportEncryption(card: AgentCard): SecurityFinding[] {
  const findings: SecurityFinding[] = [];
  const urlsToCheck: { url: string; path: string }[] = [];
 ...
 */
export function checkSsrfExposure(card: AgentCard): SecurityFinding[] {
  const findings: SecurityFinding[] = [];
  const interfaces = card.supportedInterfaces || card.supported_interfaces || [];
 ...
 */
export function checkPromptInjectionInMetadata(card: AgentCard): SecurityFinding[] {
  const findings: SecurityFinding[] = [];
  ...
  const stringsToAudit: { text: string; path: string }[] = [
 ...
 */
export function checkUnauthenticatedDangerousSkills(card: AgentCard): SecurityFinding[] {
  const findings: SecurityFinding[] = [];
 ...
 */
export function checkDeprecatedOAuthFlows(card: AgentCard): SecurityFinding[] {
  const findings: SecurityFinding[] = [];
  const schemes = card.securitySchemes || card.security_schemes || {};
 ...
 */
export function checkApiKeyQueryParameter(card: AgentCard): SecurityFinding[] {
  const findings: SecurityFinding[] = [];
  const schemes = card.securitySchemes || card.security_schemes || {};
 ...
 */
export function checkExtendedCardAuthentication(card: AgentCard): SecurityFinding[] {
  const findings: SecurityFinding[] = [];
 ...
 */
export function checkSchemaCompliance(card: AgentCard): SecurityFinding[] {
  const findings: SecurityFinding[] = [];
 ...
 */
export function executeAllRules(card: AgentCard): SecurityFinding[] {
  const findings: SecurityFinding[] = [
